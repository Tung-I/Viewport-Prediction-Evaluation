...
class HeadOrientation:
    ...
    def parse_dat2(self, _file_name):   #for Wu dataset
        temp = open(_file_name).read().split('\n')[1:-1] # each row of the csv file is a list of 7 items
        temp2 = [list(map(float, item.split(',')[1:])) for item in temp] # remove the first col (ts)
        for i, _ in enumerate(temp2):
            item = temp2[i] # a row [pb_time, q_x, q_y, q_z, q_w, hp_x, hp_y, hp_z]
            # remove head positions and get the quaternion only
            temp2[i] = [item[0], -1, item[3], item[2], item[1], item[4]] # [pb_time, -1, q_z, q_y, q_x, q_w]
        return np.array(temp2)
    ...
    def load_series_ds(self, filename_list, f_parse, dataset):
        ...
        for idx, file_name in enumerate(filename_list):
            series = f_parse(file_name);  # parse a csv file (rearrange the columns)
            # len(series) is the number of rows in the csv file

            series_ds.append(series.tolist())  # series_ds[i] = [pb_time, -1, q_z, q_y, q_x, q_w]*n_rows
        
        for uid in range(len(series_ds)):  # for each user
            t_list = set()
            tmp = []
            # remove duplicate rows that have the same playback time
            for item in series_ds[uid]:  # for each row in the csv file of the user
                t = item[0]
                if t not in t_list:
                    t_list.add(t)
                    tmp.append(item)
    ...
    def headpos_to_headvec(self, series_ds, f_extract_direction, dataset):
        ...
        for series in series_ds:
            ...
            for idx in np.arange(0, len(series)):
                item = series[idx] # item = [pb_time, -1, q_z, q_y, q_x, q_w]
                q = item[2:6]  # quaternion
                v = f_extract_direction(q)  # head direction vector [vx, vy, vz]
                if dataset==1:
                    vec.append([int(item[1]), v, 0, 0]) 
                else:
                    vec.append([item[0], v, 0, 0]) # [pb_time, v, 0, 0]
