...
class CubemapProjection(AbstractProjection):
	...
	# Get the pixel value from spherical coordinates
	def _pixel_value(self, angle):
		...
		# transform from (theta, phi) to (x,y,z) coordinates
		...
		eps = 1e-6
		# find the face of the cube that the point lies on
		if math.fabs(x)>eps:
			if x>0:
				t = 0.5/x
				u = 0.5+t*y
	...
	# Get the corresponding patch (x, y, patch_type) of the stitched image
	...
	def reprojectToEquirectangular(self, filepath, sourceProjection, width, height):
		...
		for x in objects:
			# Parse thje object detection result
			y=x.split(" ")
			frameno=y[0]
			print(frameno),
			objtype=y[2]
		...
		print(f'Save at {filepath[:-4]+"-equirectangular.txt"}')
		f_in.close()
		f_out.close()
