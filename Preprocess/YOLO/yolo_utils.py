...
def draw_labels_and_boxes(img, boxes, confidences, classids, idxs, colors, labels, filename, framenum, draw=False):
    f=open(filename,"a+")
    if len(idxs) > 0:
        for i in idxs.flatten():
            ...
            # Write the bbox details to file
...
def infer_image(net, layer_names, height, width, img, colors, labels, FLAGS, 
    ...
    img = draw_labels_and_boxes(img, boxes, confidences, classids, idxs, colors, labels, FLAGS.storefilename, FLAGS.framenum, FLAGS.draw)
